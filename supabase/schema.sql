...
-- RPC: Append a single dish to the caller's local list (Top 10) from anywhere
-- (restaurant tab, dish page) without round-tripping the whole list. The
-- rate-first gate is enforced HERE on the server so a raw RPC call can't bypass
-- it — the UI gate alone was advisory (the integrity gap noted in PR #7).
-- Mirrors the definition already live in production. Return contract:
--   added       → { success:true, already_present:false, item_count }
--   duplicate   → { success:true, already_present:true,  item_count }
--   list full   → { success:false, list_full:true, item_count, error }
--   gate/other  → { success:false, error }   (e.g. "Rate this dish first…")
